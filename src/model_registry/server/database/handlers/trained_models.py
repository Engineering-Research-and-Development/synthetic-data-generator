...
def get_trained_model_versions(model_id,version_id = None,include_training_info: bool = False) -> PydanticTrainedModelAndVersions:
    if version_id is None:
        query = ModelVersion.select().where(ModelVersion.trained_model_id == model_id)
    else:
        query = ModelVersion.select().where(ModelVersion.trained_model_id == model_id).where(ModelVersion.id == version_id)
    ...
    if include_training_info:
        ...
        versions_and_train_info = [ModelVersionAndTrainInfo(version=elem[0], training_info=elem[1]) for elem in
                         zip(versions, training_infos)]
        return PydanticTrainedModelAndVersions(**train_and_features, versions=versions_and_train_info)
    else:
