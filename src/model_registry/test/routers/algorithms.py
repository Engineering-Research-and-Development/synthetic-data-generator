...
test_algorithm = {
    ...
    "allowed_data": [
        {"datatype": "DataType_0", "is_categorical": "true"},
        {"datatype": "DataType_0", "is_categorical": "true"},
    ],
}
...
def test_create_algorithm():
    ...
    assert response.status_code == 201, print(
        local_algo["algorithm"]["name"], response.content
    )
...
def test_get_all_datatypes():
    response = requests.get(
        f"{server}:{port}{endpoint}" + "?include_allowed_datatypes=True"
    )
    assert response.status_code == 200
...
def test_get_algo_by_id_with_datatypes():
    response = requests.get(
        f"{server}:{port}{endpoint}" + "/2/?include_allowed_datatypes=True"
    )
    assert response.status_code == 200
    data = response.json()
    assert data["id"]
...
def test_delete_algo():
    local_algo = deepcopy(test_algorithm)
    local_algo["algorithm"]["name"] = local_algo["algorithm"]["name"] + str(
    ...
    assert response.status_code == 200
    assert (
        requests.get(f"{server}:{port}{endpoint}" + "/" + str(created_id)).status_code
        == 404
    )
