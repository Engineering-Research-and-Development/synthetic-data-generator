...
def test_get_train_models_and_versions_ids(client):
    data = client.get(f"{server}:{port}{endpoint}" + "?include_version_ids=True")
    assert data.status_code == 200,print(data.json())
    payload = data.json()
    # Checking that they are not empty
    assert len(payload) > 0
...
def test_get_train_model_id(client):
    payload = '/1'
    data = client.get(f"{server}:{port}{endpoint}"+ payload)
...
def test_get_bad_id(client):
    ...
    data = client.get(f"{server}:{port}{endpoint}" + "/1000")
    assert data.status_code == 404


def test_get_trained_models_and_versions(client):
    data = client.get(f"{server}:{port}{endpoint}" + "/1" + "?include_versions=True")
    assert data.status_code == 200
    payload = data.json()
    assert payload["name"]
    assert payload["id"]
...
def test_get_bad_trained_models_and_versions(client):
    data = client.get(f"{server}:{port}{endpoint}"+ "/1000" + "?include_versions=True")
    assert data.status_code == 404

def test_create_trained_model(client):
    response = client.post(f"{server}:{port}{endpoint}",json=model)
    assert response.status_code == 201, print(response.content)
    data = response.json()
    created_id = data["id"]
    assert response.status_code == 201
    # Now we check that it has been saved in the repo
    response = client.get(f"{server}:{port}{endpoint}" + "/" + str(created_id) + "?include_versions=True")
...
def test_bad_algo_id_create_trained_model(client):
    bad_data = deepcopy(model)
    bad_data["trained_model"]["algorithm_id"] = 100
    response = client.post(f"{server}:{port}{endpoint}",json=bad_data)
    assert response.status_code == 400,print(response.content)

def test_bad_datatype_create_trained_model(client):
    bad_data = deepcopy(model)
    bad_data["feature_schema"][0]["datatype"] = "BadDatatype"
    assert client.post(f"{server}:{port}{endpoint}",json=bad_data).status_code == 400

def test_delete_bad_train_model(client):
    assert client.delete(f"{server}:{port}{endpoint}" + "/100").status_code == 404

def test_delete_train_model(client):
    ...
    response = client.get(f"{server}:{port}{endpoint}" + "/" + str(model_id) + "?include_versions=True")
    assert response.status_code == 200
    data = response.json()
    ...
    assert client.delete(f"{server}:{port}{endpoint}" + "/" + str(model_id)).status_code == 200
    assert client.get(f"{server}:{port}{endpoint}" + "/" + str(model_id)).status_code == 404
    assert client.get(f"{server}:{port}{endpoint}" + "/versions/" + str(version_id)).status_code == 404
    assert client.get(f"{server}:{port}{endpoint}" + "/training_info/" + str(training_id)).status_code == 404

def test_delete_train_model_version(client):
    response = client.post(f"{server}:{port}{endpoint}", json=model)
    assert response.status_code == 201, print(response.content)
    data = response.json()
    model_id = data["id"]
    ...
    data = response.json()
    version_id = data['versions'][0]['version']['id']
    assert client.delete(f"{server}:{port}{endpoint}" + "/" + str(model_id) + "?version_id=" + str(version_id)).status_code == 200