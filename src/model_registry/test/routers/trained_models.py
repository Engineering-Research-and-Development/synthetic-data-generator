...
def test_get_bad_trained_models_and_versions():
    data = requests.get(f"{server}:{port}{endpoint}"+ "/1000" + "?include_versions=True")
    assert data.status_code == 404
...
def test_create_trained_model():
    ...
    response = requests.get(f"{server}:{port}{endpoint}" + "/" + str(created_id) + "?include_versions=True")
...
def test_delete_train_model_version():
    ...
    data = response.json()
    version_id = data['versions'][0]['version']['id']
    assert requests.delete(f"{server}:{port}{endpoint}" + "/" + str(model_id) + "?version_id=" + str(version_id)).status_code == 200