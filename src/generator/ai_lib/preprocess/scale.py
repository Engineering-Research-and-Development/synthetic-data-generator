...
def standardize_time_series(train_data:np.array, test_data:np.array=None) -> tuple[np.array, np.array, StandardScaler]:
    ...
    train_data = scaler.fit_transform(train_data.reshape(-1, features*steps)).reshape(-1, features, steps)

    if test_data is not None:
        test_data = scaler.transform(test_data.reshape(-1, features*steps)).reshape(-1, features, steps)
