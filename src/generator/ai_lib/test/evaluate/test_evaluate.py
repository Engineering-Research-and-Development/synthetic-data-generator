...
def test_evaluate_adherence(evaluator_correct):
    ...
    assert len(report["category_adherence_score [%]"]) == len(evaluator_correct._categorical_columns)
    assert len(report["boundary_adherence_score [%]"]) == len(evaluator_correct._numerical_columns)
