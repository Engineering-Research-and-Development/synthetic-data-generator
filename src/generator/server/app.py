...
async def train(request: TrainRequest):
    ...
    for algo in generator_algorithms:
        if algo['name'] == request["model"]['algorithm_name']:
            flag = True
    if not flag:
        return JSONResponse(status_code=404, content="This algorithm is not implemented by the generator!")
    # Here we calculate the unique name of the folder
    folder_id = (request["model"]["model_name"] + trim_name(request["model"]['algorithm_name'])
                 + datetime.now().strftime("%Y%m%d.%H%M%S"))
    folder_path = Path(os.path.join(APP_FOLDER,Path(f"saved_models/trained_models/{folder_id}")))
    # Here we create a temp directory
    with tempfile.TemporaryDirectory(dir=os.path.dirname(os.path.abspath(__file__))) as tmp_dir:
        try:
            ...
            return JSONResponse(status_code=500, content="Something went wrong during training")
    ...
    add_couch_data(couch_doc,new_data={"results": results,
                                                 "metrics": metrics,
